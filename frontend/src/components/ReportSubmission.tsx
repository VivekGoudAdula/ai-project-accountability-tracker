...
const ReportSubmission = ({ subjectId, onSuccess }: Props) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!description.trim()) {
      toast.error('Please write a project report description.');
      return;
    }
    setSubmitting(true);
    try {
  ...
  return (
    <motion.form onSubmit={handleSubmit} className="space-y-5" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      <div className="flex items-center gap-3 mb-2">
        ...
        <div>
          <p className="text-sm font-bold text-foreground">Project Report</p>
          <p className="text-[11px] text-muted-foreground">Upload your final project report PDF</p>
        </div>
      </div>

      <div className="space-y-2">
        <label className="text-xs font-bold text-foreground uppercase tracking-wider">Project Report PDF</label>
        <div
          className={cn(
        ...
        >
          ...
          {files.length === 0 ? (
            <div className="flex flex-col items-center gap-2 text-center py-3">
              ...
              <p className="text-sm font-semibold text-muted-foreground group-hover:text-foreground">Upload Project Report</p>
              <p className="text-[11px] text-muted-foreground">PDF or DOCX up to 10MB</p>
            </div>
          ) : (
      ...
      <div className="space-y-2">
        <label className="text-xs font-bold text-foreground uppercase tracking-wider">Report Abstract</label>
        <Textarea
          placeholder="Briefly describe the contents of your project report, key contributions, and conclusions..."
          rows={4}
          value={description}
      ...
      <Button type="submit" className="w-full h-12 gap-2 font-bold text-base shadow-lg shadow-primary/20" disabled={submitting}>
        {submitting ? <><Loader2 className="h-5 w-5 animate-spin" /> Submitting & Evaluating...</> : <><CheckCircle2 className="h-5 w-5" /> Submit Project Report</>}
      </Button>
    </motion.form>
  );
};
